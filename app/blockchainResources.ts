...
export async function signMessage(message: string){
  try {
    ...
    const signer = blockchainResources.provider && await blockchainResources.provider.getSigner();
    const addr = signer && await signer.getAddress();
    //@ts-ignore
    const sig = blockchainResources.provider && await blockchainResources.provider.send('personal_sign', [ethers.utils.hexlify(data), addr.toLowerCase()]);
    return sig;
  }
  catch(e){
    throw e;
